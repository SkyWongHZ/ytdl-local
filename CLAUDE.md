...
`ytdl-local` is a minimal local Node.js CLI for downloading **one** YouTube video at a time.
...
- `ffmpeg` installed via Homebrew, verified at version `8.1.1`
...
Always quote YouTube URLs in zsh because `?` can be interpreted by the shell.
...
- Optional `--cookies-from-browser <browser>`: `chrome`, `safari`, `firefox`, or `edge`.
...
Reason: a previous 4K AV1 download produced a white screen with audio only in some macOS players. VP9 4K is preferred because it is more likely to play correctly while keeping high resolution.
...
This can produce much larger files. It is **not** upscaling; it only selects a better source stream when YouTube offers one.
...
If a video plays with audio only, inspect the file:
...
Avoid downloading large files unless the user explicitly requests a real download test.
